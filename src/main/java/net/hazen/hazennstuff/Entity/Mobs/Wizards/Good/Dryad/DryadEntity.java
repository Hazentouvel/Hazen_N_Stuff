...
public class DryadEntity extends NeutralWizard implements VillagerDataHolder, SupportMob, HomeOwner, IMerchantWizard {
    private static final EntityDataAccessor<VillagerData> DATA_VILLAGER_DATA = SynchedEntityData.defineId(DryadEntity.class, EntityDataSerializers.VILLAGER_DATA);
    private static final EntityDataAccessor<Boolean> DATA_VILLAGER_UNHAPPY = SynchedEntityData.defineId(DryadEntity.class, EntityDataSerializers.BOOLEAN);
    public GoalSelector supportTargetSelector;
    private int unhappyTimer;

    public DryadEntity(EntityType<? extends AbstractSpellCastingMob> pEntityType, Level pLevel) {
...
//        }
    ...
    public MerchantOffers getOffers() {
        if (this.offers == null) {
            ...
            this.offers.add((new AdditionalWanderingTrades.RandomScrollTrade(new SpellFilter((SchoolType) SchoolRegistry.NATURE.get()), 0.0F, 0.25F)).getOffer(this, this.random));
            if (this.random.nextFloat() < 0.8F) {
                this.offers.add((new AdditionalWanderingTrades.RandomScrollTrade(new SpellFilter((SchoolType)SchoolRegistry.NATURE.get()), 0.3F, 0.7F)).getOffer(this, this.random));
            }

            if (this.random.nextFloat() < 0.8F) {
                this.offers.add((new AdditionalWanderingTrades.RandomScrollTrade(new SpellFilter((SchoolType)SchoolRegistry.NATURE.get()), 0.8F, 1.0F)).getOffer(this, this.random));
            }


            this.offers.add(new NatureSpellbookTrade().getOffer(this, this.random));
    ...
    static class NatureSpellbookTrade extends AdditionalWanderingTrades.SimpleTrade {
        private NatureSpellbookTrade() {
            super((trader, random) -> {
                if (!trader.level().isClientSide) {
                    ItemStack cost = new ItemStack(ItemRegistry.CHAINED_BOOK.get());
                    ItemStack forSale = new ItemStack(ItemRegistry.DRUIDIC_SPELL_BOOK.get());
